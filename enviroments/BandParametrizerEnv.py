...
class RunEnv(BacktestEnvSpot):
  ...
  def reset(self, postition_ratio=1.0, leverage=1, StopLoss=0.01, typeMA=0, MA_period=1, ATR_period=1, ATR_multi=1.000):
     ...
     super().reset()
     collect()
     return np.array([-1.0 for _ in range(8)], dtype="float32")
  
  def _finish_episode(self):
     _, reward, done, info = super()._finish_episode()
     obs =  np.array([self.episode_orders, self.cumulative_fees, self.liquidations, self.sharpe_ratio, self.sortino_ratio, self.max_drawdown, self.trades_ratio, self.pnl_means_ratio])
...
class BandParametrizerEnv(Env):
    ...
    def reset(self, postition_ratio=1, leverage=1, StopLoss=0.07, typeMA=0, MA_period=2, ATR_period=2, ATR_multi=1):
        ...
        #print(get_attributes_and_deep_sizes(self.running_enviroment))
        return self.running_enviroment.reset(postition_ratio, leverage, StopLoss, typeMA, MA_period, ATR_period, ATR_multi)
    ...
    def step(self, action):
      ...
      #self.reset(postition_ratio=round(action[0], 2), leverage=int(action[1]), StopLoss=round(action[2],4), typeMA=int(action[3]), MA_period=int(action[4]), ATR_period=int(action[5]), ATR_multi=round(action[6], 3))
      self.reset(postition_ratio=1, leverage=1, StopLoss=round(action[0],4), typeMA=int(action[1]), MA_period=int(action[2]), ATR_period=int(action[3]), ATR_multi=round(action[4], 3))
      return self.running_enviroment._execute()